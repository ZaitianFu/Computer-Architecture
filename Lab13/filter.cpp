...
filter_fusion(pixel_t **image1, pixel_t **image2) {
  /*
  for (int i = 1; i <= 3*SIZE-12; i++) {
    if (i<= SIZE-2){
      filter1(image1, image2, i);
    }
    else if(i <= 2*SIZE-6){
      filter2(image1, image2, i-SIZE +3);
    }
    else {
      filter3(image2, i-(2*SIZE)+6);
    }
  }
   */
...
filter_prefetch(pixel_t **image1, pixel_t **image2) {
    ...
    for (int i = 1; i < SIZE - 1; i ++) {
        filter1(image1, image2, i);
      __builtin_prefetch((image1[i+15]), 0, 1);
      __builtin_prefetch((image2[i+15]), 1, 3);
    }
    ...
    for (int i = 1; i < SIZE - 5; i ++) {
        filter3(image2, i);
      __builtin_prefetch((image2[i+15]), 0, 3);
    }
}
